...
| **P3 · relevance ranking + DSH tool vocabulary** | `core/rank.ts` (`scoreBlock`, `dedupKey`, `selectRankedBriefBlocks`, `EDIT_TOOL_RE` / `TEST_COMMAND_RE` / `WORKFLOW_COMMAND_RE` / `TRIVIAL_BASH_LINE_RE`) | Upstream ranks normalized blocks and keeps the best ones under a size budget; here every row is compiled first, so the ranking decides which row the cap drops first (`src/rank.js` → `lowestValueIndex`, replacing oldest-first in Phase A) and which repeated rows collapse (`collapseRepeatedRows`: runs of ≥3 keep first + last, the middle becomes one `note` marker naming its seq range). The value model is upstream's — recency, kind, tool shape (edit 34 > test 26 > workflow 14 > other 12 > read 6 > scaffolding −16), repeat penalty — with the scaffolding regexes extended to PowerShell (`Set-Location`, `Get-ChildItem`, `Write-Host`, `Start-Sleep`). Two correctness fixes came out of it: the elision marker now **reserves its own size** before rows are dropped (a note must not evict conversation text to pay for itself), and `seqRangeOf` sorts, because value-order dropping produced markers like `seqs 5-2` that `recall` rejects as malformed. The DSH vocabulary gap the ranking exposed is fixed in `DEFAULT_TOOL_KEY_FIELDS` / `DEFAULT_ARG_TOOLS`: `pwsh` (this host's shell tool — 25,826 of 70,424 tool rows across ten sessions) and `ssh_exec`, `sftp_*`, `web_fetch_pro`, `web_search_pro`, `web_platform_search`, `browser_open/crawl`, `recall`, `search`, `touched_files` now render their key argument. Config: `rankElision` (default true). | `src/rank.js` (new), `src/compiler.js`, `src/index.js`, `src/client.js`, `package.json` (`./rank` export + `check`), `types/rank.d.ts` (new), `types/index.d.ts`, `test/rank.test.js` (new), `test/compiler.test.js`, `test/config.test.js` |

**P3 measured over 112 real compaction regions** (same regions, same budgets): tool rows carrying a
payload **32.8% → 76.4%**; compiled size **+3.4%**; under a 40% cap high-value rows retained
**11.6% → 19.8%** with conversation text untouched at **100%** in both.

**Deliberately not ported from P3**: `core/skill-collapse.ts`. Its `<skill name="…">…</skill>`
collapsing has no call site here — measured over 228,671 compiled rows from ten sessions, the pattern
occurs **0 times**: DSH delivers skill bodies through the skill-catalog injection (dropped from the
compiled view by P2a) and through tool results, which do not occupy entries. `core/drill-down.ts` (the
`#N:path[:full|:offset:limit]` query syntax) is also not ported: `recall` already restores a tool call's
full arguments and its result by pointer, and adding a second addressing syntax on top of the reference
space unified in P2b would recreate the drift that port exists to remove. `core/brief.ts`'s renderers
are Pi-shaped; the portable half — block *selection* — is what the ranking above ports.
