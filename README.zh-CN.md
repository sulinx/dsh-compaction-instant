...
| 共享核心 | `dsh-compaction-instant/recall` + `/search` + `/format` | 日志展开、预算、字段筛选；正则编译（含防灾难回溯）、BM25-lite 排序、片段与路径展示 |
...
搜索正则是调用方（模型或人）给的，而正则可以构造成指数级回溯——在单线程宿主上一条 `(a+)+$` 就能把共享该进程的所有会话卡死。对"组内已含无界量词、组外又套无界量词"的模式（如 `(a+)+`）改为**字面量匹配**（命中行头部会注明，查询照样有答案）；**墙钟预算**（`searchBudgetMs`，默认 **3000**）超过即中止，检查点位于每个事件之间与每 512 行，因此结构判断看不见的模式（如 `(a|a)+`）最多跑满一个检查点窗口，而不是整个语料。正文走 indexOf 行扫描器而不是 `split("\n")`，排序路径只保留每个事件的词频而不保留文档正文，因此 15 MB 的会话搜索约 100–220 ms，临时分配远小于 1 MB。
...
**行是有相关性排序的，所以空间紧张时预算花在该花的地方。** 上游 `core/rank.ts` 给规范化块打分再挑最好的；这里每一行都先编译出来（不静默丢弃），排序决定的是**检查点超限时先丢哪一行**，以及哪些重复行可以折叠。行的价值 = 新近度 + 类型（用户正文 > 助手正文 > 工具 > 媒体/备注）+ 工具形状（编辑 **34** > 测试命令 **26** > 工作流命令 **14** > 其他 **12** > 读取 **6** > 脚手架 **−16**，POSIX 与 PowerShell 两种写法的 `cd`/`ls`/`echo`/`sleep` 都认）+ 重复惩罚。连续 3 次以上完全相同的工具行只保留首尾，中间折叠成一条 `note` 标记并写明 seq 区间，`recall` 可完整取回。`rankElision: false` 可退回旧的「从最旧的低价值行开始丢」。

排序还带来一处词表修正：本机宿主的外壳工具是 `pwsh`，原先只渲染名字——这种行只记录"跑过一条命令"，不记录"跑了什么"。现在 `pwsh`、`ssh_exec`、`sftp_*`、`web_fetch_pro`、`web_search_pro`、`web_platform_search`、`browser_open/crawl`、`recall`、`search`、`touched_files` 都会带参数。实测 **112 个真实压缩区间**：

| | 改前 | 改后 |
|---|---|---|
| 工具行带载荷比例 | 32.8% | **76.4%** |
| 编译体积（同样区间、不限上限） | — | **+3.4%** |
| 40% 上限下保留的高价值行 | 11.6% | **19.8%** |
| 40% 上限下保留的对话正文 | 100% | 100% |

省略标记现在还会**先预留自身体积**再丢行（一条关于工具行的备注不该把对话正文挤掉来给自己腾地方），并且在按价值顺序丢行的情况下仍按 min–max 报告区间——降序的 `seqs 5-2` 是个粘不回去的指针。

**引擎打印出来的每一个指针，都是它能解析的指针。** 引用空间由 `indices` 统一定义——seq 如何映射到事件、检查点如何计数、打印出来的标记如何解析回来——因此文件视图渲染的 `#N`、检查点里的 `(seq N)`/`(seqs A-B)`、工具行尾的 `-> result N`、以及 `[checkpoint N]` 序号，全都可以原样粘贴回 `recall`。比展开分片更宽的区间会**分片展开而不是拒绝**：一条 `[N entries elided: seqs A-B]` 标记如果粘不回去，那就是个不起作用的指针（改为对总展开量设上限，100000 个 seq）。在十份真实会话日志上实测：**66,571 个指针，0 个无法解析**——唯一例外是下面这种 seeded 会话。

**从别的会话日志带进来的检查点会被明确标注，而不是悄悄指错。** 由父会话播种的会话（`isSeeded`，例如交接/续接会话）会把父会话已落地的检查点原样带进来，而那些指针指向的是**父会话**的编号空间——实测其中一份 seeded 会话，继承文本里约 2.8k 个指针中有 273 个属于这种情况，其余会话解析率是 100%。编译器无法改写它们（父会话日志可能已不存在，且文本必须保持逐字），所以它在那段嵌套内容前加一行：*"这些 seq 指针来自另一个会话的日志；用 `search` 找回这段内容"*。`search` 照样能搜到该内容，本会话内新生成的指针不受影响。

被保留的尾部现在**有账可查**：选择阶段会记录是哪条规则留下的它——`retain-turns`（优先回合数规则）、`whole-turns`（上限仍装得下整回合）、`node-suffix`（最新回合本身就超过上限，只保留装得下的部分）——以及工具配对守卫是否回退过。检查点页脚会打印该信息，调试行里带 `tail=<策略>/kept=<节点数>nodes/<token>tok/receded=<bool>`，所以一个异常大或异常小的原文尾部，单看转录就能解释清楚（上游 pi-vcc 记录的同类出处字段叫 `budgetCut`）。
