...
|---|---|---|
| `recall` **tool** (model-facing) | `dsh-compaction-instant/tool` | Typed restore: `type:"seq"` with `(seq N)`/`(seqs A-B)`/`#N` markers, `type:"result"` with the `result N` pointer, `type:"checkpoint"` with a `[checkpoint N]` ordinal — restores exact original content into the current tool result |
| `search` **tool** (model-facing) | `dsh-compaction-instant/tool` | Keyword, regex or natural-language search over the whole durable log — including content elided by compaction — returning matching events with their `(seq N)` pointers, ready for `recall` |
| `touched_files` **tool** (model-facing) | `dsh-compaction-instant/tool` | Every file the session read/wrote/edited, aggregated by path with the seq pointers of each operation, 5 per page |
| `/recall` **command** (human) | `dsh-compaction-instant/command` | `/recall <keyword\|regex\|prose>` appends a durable `form: "recall"` user message with the matching events and their seq pointers, so the next model turn sees them; `/recall files [page]` does the same for the files-touched view |
| Shared reference space | `dsh-compaction-instant/indices` | One definition of seq lookup, checkpoint ordinals, and the parser for every printed pointer — the printing side (compiler) and the resolving side (`recall`/`search`) cannot drift |
| Shared cores | `dsh-compaction-instant/recall` + `/search` + `/format` | Seq parsing, log expansion, budgets, projection; ReDoS-guarded pattern compilation, BM25-lite ranking, snippet and path rendering |

Recall keeps **everything**: text, reasoning, raw tool-call arguments, nested tool-result content; log-only events render as labeled data dumps; missing seqs are reported; a `maxRecallTokens` budget (default **16000**) cuts with a provenance marker and counts the skipped remainder; searches cap shown hits (`maxSearchHits`, default **50**).

Search has two modes, chosen from the query itself. A single word, or anything containing regex metacharacters, is scanned as one pattern and every matching event renders its matching lines with line numbers. A **multi-word query** is scored with BM25-lite over the whole corpus and renders the best events first, each with a numbered context window around its first match — so *"why does the counter overflow"* is answered by the events that discuss it instead of by every event containing one of its words. Ranked results drop the OR-tail below **20%** of the top score, but only when the query has **two or more distinct** effective terms (repeating or re-casing one word is still a single-term query). Every withheld match is reported honestly (`N of M matching event(s) shown`, `N low-relevance match(es) hidden`, `[N more matching event(s) omitted]`) instead of quietly understating the count. A prose question whose trailing `?` or `.` made the pattern path match nothing verbatim falls through to the ranked path rather than answering "no matches" (upstream measured that gate at 47.5% → 1.1% zero-hit queries on real sessions).

A search never matches **its own surface**: `/recall` output is a durable user message and the `recall`/`search`/`touched_files` tools leave their invocation arguments and results in the log, so without excluding all three a repeated query keeps matching its own earlier output and the hit list grows on every search — on one real session that was 40 of 43 shown hits.

Search patterns are caller-supplied, and a regular expression can be made to backtrack exponentially — on a single-threaded host one `(a+)+$` would otherwise freeze every session sharing the process. A pattern that applies an unbounded quantifier to a group that already contains one is matched **literally** (the hit header says so, and the query still answers), and a wall-clock budget (`searchBudgetMs`, default **3000**) aborts a search that outruns it — checked between events and every 512 scanned lines, so a pattern the structural guard cannot see (`(a|a)+`) stops after one checkpoint window instead of the whole corpus. Bodies are walked with an indexOf line scanner rather than `split("\n")`, and the ranked path keeps per-event term counts instead of document text, so a 15 MB session searches in ~100–220 ms with well under a megabyte of transient allocation.
...
The retained tail is **accounted for, not just produced**: the selection records which rule kept it — `retain-turns` (preferred-turn rule), `whole-turns` (the ceiling still fit whole turns) or `node-suffix` (the newest turn is bigger than the ceiling, so only the part that fits is kept) — plus whether the tool-pairing guard had to recede. The checkpoint footer prints it and the debug line carries `tail=<policy>/kept=<n>nodes/<t>tok/receded=<bool>`, so an unexpectedly large or small verbatim tail is explainable from the transcript alone (upstream pi-vcc records the same provenance as `budgetCut`).
...
The tool and command plugins each take their own `{ maxRecallTokens?: 16000, maxSearchHits?: 50, searchBudgetMs?: 3000 }` config. Both are separate rows, so they can be mounted next to **any** compaction backend — they only read the durable log. The ranked search path's relative floor (`0.2`) is a tuned constant, not a config key: production call sites always use the default, and only benches and targeted tests pass an override programmatically (mirroring upstream's `SearchTuning`).

> **Cordis config gotcha:** the plugin row's config passes through the schemastery schema, whose `~standard` adapter injects **`[]` for every absent array key** (`toolArgTools`, `hideTools`, `skipInjectTypes`, `noisePatterns`, `toolKeyFields`, `modelPolicies`). The resolver treats an empty list as *unset* and falls back to the defaults — so a missing `toolArgTools` keeps the built-in whitelist (never disable it by writing `toolArgTools: []`; empty means default). `debug: true` writes per-compile diagnostics to the configured `debugLogPath` (default `$DSH_HOME/compaction-debug.log`).
>
> **Why per-turn injections are skipped by default (measured):** compiling the same compaction span with the filter off and on, across three real sessions, **73%–98% of the compiled budget was per-turn boilerplate** (one 27-node span: 8240 → 1042 tokens, 87.4% saved; a 13-node span saved 95.5%). Whole-session sizing: the runtime-context snapshot is ~71 KB per turn and the skill catalog ~16.6 KB per turn — 6.8 MB in one 15.5 MB session, present in 74/74 of its checkpoints. Turning the switch off restores the previous behavior.
